...
import com.example.gerenciador_tarefas.dto.response.TarefaHistoricoResponse;
...
//método só pode ser feito por gestor e o gestor só pode pesquisar se estiver ativo e não estiver de férias
    ...
    public TarefaHistoricoResponse historicoTarefa(String idTarefa){

        Tarefa tarefa = repository.findById(idTarefa)
                .orElseThrow(() -> new TarefaNaoEncontradaException());

        Historico historico = historicoRepository.findByTarefa(tarefa)
                .orElseThrow(() -> new HistoricoNaoEncontradoException());

        return new TarefaHistoricoResponse(
                historico.getTarefa().getNome(),
                historico.getTarefa().getUsuario().getNome(),
                historico.getStatusHistorico().getStatus(),
                historico.getStatusHistorico().getData());
